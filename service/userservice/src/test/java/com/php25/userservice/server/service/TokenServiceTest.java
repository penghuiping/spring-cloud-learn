...
public class TokenServiceTest extends BaseServiceTest {
    ...
    @Test
    public void test() {
        Map<String, String> map = tokenService.generateToken(objId);
        ...
        Assert.assertEquals("验证accessToken的失效时间是配置的失效时间失败", tokenService.getAccessTokenExpireTime(), access_token_expire_time);
        Assert.assertEquals("验证refreshToken的失效时间是配置的失效时间失败", tokenService.getRefreshTokenExpireTime(), refresh_token_expire_time);

        Assert.assertTrue("验证accessToken是有效的失败", tokenService.checkTokenValidation(accessToken, String.class));

        Map<String, String> map1 = tokenService.getToken(refreshToken);
        ...
        Assert.assertFalse("验证accessToken变为失效失败", tokenService.checkTokenValidation(accessToken, String.class));

        Assert.assertTrue("根据对象id清除token与refreshToken成功失败", tokenService.cleanToken(objId));
