...
import com.php25.usermicroservice.web.vo.req.ReqUnableGroupVo;
...
public class GroupController extends JSONController {
    ...
    public JSONResponse unableGroup(@RequestAttribute String appId, @Valid @RequestBody ReqUnableGroupVo reqUnableGroupVo) {
        Boolean result = groupService.unableGroup(appId, reqUnableGroupVo.getGroupId());
        return succeed(result);
    }

    @PostMapping("/create")
    public JSONResponse createRole(@RequestAttribute String appId, @Valid @RequestBody ReqCreateGroupVo reqCreateGroupVo) {
        GroupCreateDto groupCreateDto = new GroupCreateDto();
        groupCreateDto.setName(reqCreateGroupVo.getName());
        groupCreateDto.setDescription(reqCreateGroupVo.getDescription());
        Boolean result = groupService.create(appId, groupCreateDto);
        return succeed(result);
    }

    @PostMapping("/changeInfo")
    public JSONResponse changeInfo(@RequestAttribute String appId, @Valid @RequestBody ReqGroupChangeInfoVo reqGroupChangeInfoVo) {
        Boolean result = groupService.changeInfo(appId, reqGroupChangeInfoVo.getGroupId(), reqGroupChangeInfoVo.getDescription());
        return succeed(result);
    }
